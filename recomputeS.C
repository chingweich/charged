...
void recomputeS(std::string fin)
{
  //TString endfix1=gSystem->GetFromPipe(Form("file=%s; echo \"${file##*dytw}\"",fin.data()));
  TString  endfix1=gSystem->GetFromPipe(Form("file=%s; test=${file##*dytw}; echo \"${test%%.root*}\"",fin.data()));
  TString endfix=gSystem->GetFromPipe(Form("file=%s; echo \"${file##*.root}\"",endfix1.Data()));
  ...
  for(int k=0; k<2; k++)
    {
      ...
      hlost[k] = new TH1F(Form("hlost_%s%s",title[k].data(),endfix.Data()),Form("Fraction of Non-digitized Hits Relative to All Hits in %s",title[k].data()),nLayers[k],0.5,(float)(nLayers[k]+0.5));
  ...
  TFile* outFile = new TFile("allHlost.root","update");       
  for(int i=0;i<2;i++){
